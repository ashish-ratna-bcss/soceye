...
async function runPassA(text, log, matchedKeywords = []) {
  ...
  if (mode === 'SENTIMENT_SERVICE') {
    ...
    const result = await intelligenceClient.analyzeText(text, { lane: 'bulk' }, matchedKeywords);
  ...
  if (mode === 'SHADOW' && llmResult && Math.random() < SHADOW_SAMPLE_RATE) {
    ...
    Promise.resolve()
      .then(() => intelligenceClient.analyzeText(text, { lane: 'bulk' }, matchedKeywords))
      .then((candidate) => {
...
const analyzeContent = async (text, options = {}) => {
  ...
  try {
    ...
    let { llmResult, sentimentFromIntel, mode } = await runPassA(text, log, options.matchedKeywords || []);
