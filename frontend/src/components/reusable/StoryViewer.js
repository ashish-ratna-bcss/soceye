...
const buildCandidates = (s) => {
  ...
  if (isVid) {
    ...
    if (s.video_url || s.videoUrl) candidates.push(s.video_url || s.videoUrl);
    if (s.original_url) candidates.push(s.original_url);
  } else {
    if (s.s3_url) candidates.push(s.s3_url);
    if (s.original_url) candidates.push(s.original_url);
    if (s.thumbnail_url) candidates.push(s.thumbnail_url);
    if (s.display_url) candidates.push(s.display_url);
    if (s.image_url) candidates.push(s.image_url);
  }
