...
async function generateEventKeywords(eventData) {
  if (!GROQ_API_KEY) {
    logger.info('[KeywordGen] GROQ_API_KEY not set');
    return null;
  }
  ...
  try {
    ...
    logger.info('[KeywordGen] Step 1: Web search (compound-mini)...');
    let searchResults = '';
    try {
      searchResults = await webSearch(eventData);
      logger.info(`[KeywordGen] Search found ${searchResults.length} chars in ${((Date.now() - startTime) / 1000).toFixed(1)}s`);
    } catch (searchErr) {
      logger.error('[KeywordGen] Web search failed, proceeding with LLM only:', searchErr.response?.data?.error?.message || searchErr.message);
    }

    // Step 2: Generate keywords with 70b, grounded by search data (~2-5s)
    logger.info('[KeywordGen] Step 2: Generating keywords (70b)...');
    const structured = await generateFromSearch(searchResults, eventData);
    const total = ((Date.now() - startTime) / 1000).toFixed(1);
    logger.info(`[KeywordGen] Done in ${total}s`);

    const result = extractJSON(structured);
    if (!result?.keywords_by_language) {
      logger.error('[KeywordGen] Parse failed:', structured?.substring(0, 300));
      return null;
    }
    ...
    if (status === 429) {
      logger.info(`[KeywordGen] Rate limited after ${elapsed}s:`, errMsg);
      return null;
    }
    logger.error(`[KeywordGen] Failed after ${elapsed}s (${status}):`, errMsg);
    return null;
  }
}
