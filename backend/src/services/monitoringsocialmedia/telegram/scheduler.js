...
const TICK_MS = Number(process.env.TELEGRAM_MONITOR_TICK_MS) || 60_000;
...
const tick = async () => {
  ...
  try {
    ...
    for (const dbName of dbNames) {
      ...
      try {
        const due = await loadDueTelegramAccounts(tenantPrisma);
        for (const row of due) {
          const key = flightKey(dbName, row.id);
          if (inFlight.has(key)) continue;
          inFlight.add(key);
          try {
            await runTelegramProfile(row.id, {
              force: false,
              db: tenantPrisma,
              dbName,
            });
        ...
        console.error(
          `[monitoringsocialmedia/telegram] scheduler tenant=${dbName}:`,
          error.message
        );
      }
    }
  } catch (error) {
    console.error('[monitoringsocialmedia/telegram] scheduler tick:', error.message);
  } finally {
    ticking = false;
  }
};
