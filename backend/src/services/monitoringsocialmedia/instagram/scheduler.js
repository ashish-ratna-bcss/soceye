...
const loadDueInstagramAccounts = async () => {
...
const tick = async () => {
  ...
  ticking = true;
  try {
    const due = await loadDueInstagramAccounts();
    for (const row of due) {
      if (inFlight.has(row.id)) continue;
      inFlight.add(row.id);
      try {
        await runInstagramProfile(row.id, { force: false });
      } finally {
...
const startScheduler = () => {
  if (timer) return;
  console.log('[monitoringsocialmedia/instagram] scheduler started');
