...
const loadDueFacebookAccounts = async (prisma) => {
...
const tick = async () => {
  ...
  try {
    ...
    for (const dbName of dbNames) {
      ...
      try {
        const due = await loadDueFacebookAccounts(tenantPrisma);
        for (const row of due) {
          const key = flightKey(dbName, row.id);
          if (inFlight.has(key)) continue;
          inFlight.add(key);
          try {
            await runFacebookProfile(row.id, {
              force: false,
              db: tenantPrisma,
              dbName,
            });
        ...
        console.error(
          `[monitoringsocialmedia/facebook] scheduler tenant=${dbName}:`,
          error.message
        );
      }
    }
