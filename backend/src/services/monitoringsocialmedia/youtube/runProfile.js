...
const { fetchYouTubePosts } = require('./fetch');
...
const runYouTubeProfile = async (accountId, opts = {}) => {
  ...
  if (account.platforms?.slug !== 'youtube') {
    return { ok: false, skipped: true, reason: 'not_youtube' };
  }
  ...
  try {
    ...
    const { authFromPlatformRow } = require('../../blugate/youtube/blugate.youtube.api_client');
    const auth = authFromPlatformRow(account.platforms);
    const { posts, apiHits: hits, dataPatch } = await fetchYouTubePosts(account, auth);
    apiHits = hits;
...
module.exports = { runYouTubeProfile };
