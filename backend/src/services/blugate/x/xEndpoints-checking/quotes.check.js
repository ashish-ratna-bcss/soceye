// Checks QUOTES against a real tweet id pulled from @elonmusk's timeline.
// If this tweet is ever deleted, grab a fresh id from user_tweets.check.js's output.
// Saves its result to output/output.quotes.json (overwritten each run).
// Run: node quotes.check.js
...
const OUTPUT_FILE = path.join(__dirname, 'output', 'output.quotes.json');
...
(async () => {
    ...
    try {
        ...
        console.error('QUOTES: FAIL —', err.message);
        process.exitCode = 1;
    } finally {
