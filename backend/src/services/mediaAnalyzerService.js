...
async function isMediaAnalyzerAvailable() {
  try {
    await axios.get(`${MEDIA_ANALYZER_URL}/health`, {
      timeout: 3000,
...
// HLS is streamable but never directly downloadable — the URL points at a
// playlist, not the video. Kept separate from isDirectMediaUrl() because
// playback (toStreamProxyUrl / hls.js) legitimately needs .m3u8 to pass.
function isHlsUrl(rawUrl) {
  const parsed = safeParseUrl(rawUrl);
  if (!parsed) return false;
  return /\.m3u8(\?|$)/i.test(parsed.pathname || '') || /\.m3u8/i.test(String(rawUrl));
}
...
async function downloadVideo(mediaUrl) {
  ...
  // An HLS URL is a ~1 KB text playlist, never a downloadable file. Handing it
  // back produces a .mp4 that Windows/WhatsApp reject as corrupt, which is
  // worse than failing — an officer can forward it believing it is evidence.
  if (isHlsUrl(normalizedUrl)) {
    const err = new Error('This video is only available as an adaptive (HLS) stream and cannot be downloaded directly. Play it in the app instead.');
    err.statusCode = 422;
    throw err;
  }
