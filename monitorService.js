...
const performFullAnalysis = async (content, settings, keywords, options = {}) => {
  try {
    ...
    if (matchedKeywords.length > 0) {
      ...
      // 3. Log contextual assessment (Risk Override has been removed)
      if (analysisData.keyword_context && analysisData.keyword_context.length > 0) {
        analysisData.keyword_context.forEach(ctx => {
          logger.info(`[Analysis] Keyword context evaluated for '${ctx.keyword}': relevant=${ctx.contextually_relevant}, usage=${ctx.usage}`);
        });
      }
    }
